...
stdenv.mkDerivation (finalAttrs: {
  ...
  preConfigure =
    ...
    # note! Nix does not set CMAKE_INSTALL_DATADIR to an absolute path, so this replacement would be incorrect
      ...
      substituteInPlace src/utils/CMakeLists.txt \
        --replace-warn '$'{RETDEC_GIT_VERSION_TAG} ${finalAttrs.version} \
        --replace-warn '$'{RETDEC_GIT_COMMIT_HASH} ${finalAttrs.src.rev}
  ...
  passthru.full = buildEnv {
    name = "${finalAttrs.pname}-full-${finalAttrs.version}";
    inherit (finalAttrs.finalPackage) meta;
    paths = [ finalAttrs.finalPackage.out finalAttrs.finalPackage.patterns ];
    postBuild = ''
